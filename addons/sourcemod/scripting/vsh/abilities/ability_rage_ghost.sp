...
static Handle g_hGhostTimer[TF_MAXPLAYERS+1];
...
methodmap CRageGhost < SaxtonHaleBase
{
	...
	public CRageGhost(CRageGhost ability)
	{
		...
		g_hGhostTimer[ability.iClient] = null;
	...
	public void OnRage()
	{
		...
		this.CallFunction("KillTimer", g_hGhostTimer[this.iClient]);
		g_hGhostTimer[this.iClient] = this.CallFunction("CreateTimer", this.flDuration, "CRageGhost", "OnRageEnd");
	}
	...
	public void OnThink()
	{
		...
		for (int iVictim = 1; iVictim <= MaxClients; iVictim++)
		{
			...
			if (SaxtonHale_IsValidAttack(iVictim) && IsPlayerAlive(iVictim))
			{
				...
				if (GetVectorDistance(vecOrigin, vecTargetOrigin) <= flRadius)
				{
					...
					if (g_flGhostHealStartTime[this.iClient][iVictim] == 0.0)
					{
						g_flGhostHealStartTime[this.iClient][iVictim] = GetGameTime();
						
						vecTargetOrigin[2] += 42.0;
						float vecTargetAngles[3];
						GetClientAbsAngles(this.iClient, vecTargetAngles);
						g_iGhostParticleBeam[this.iClient][iVictim] = TF2_SpawnParticle(sParticle[iTeam], vecTargetOrigin, vecTargetAngles, true, iVictim, EntRefToEntIndex(g_iGhostParticleCentre[this.iClient]));
					}
					
					//Calculate on heal steal
					float flTimeGap = GetGameTime() - g_flGhostHealStartTime[this.iClient][iVictim];
					
					int iExpectedSteal = RoundToCeil(flTimeGap * flHealSteal);
					if (iExpectedSteal > g_iGhostHealStealCount[this.iClient][iVictim])
					{
						float flDamage = float(iExpectedSteal - g_iGhostHealStealCount[this.iClient][iVictim]);
						SDKHooks_TakeDamage(iVictim, this.iClient, this.iClient, flDamage, DMG_PREVENT_PHYSICS_FORCE);
						g_iGhostHealStealCount[this.iClient][iVictim] = iExpectedSteal;
					}
					...
					if (iExpectedGain > g_iGhostHealGainCount[this.iClient][iVictim])
					{
						Client_AddHealth(this.iClient, iExpectedGain - g_iGhostHealGainCount[this.iClient][iVictim]);
						g_iGhostHealGainCount[this.iClient][iVictim] = iExpectedGain;
					}
				}
			}
				
			//Check if beam ent need to be killed, from out of range or client death/disconnect
			if (!bSpook && g_flGhostHealStartTime[this.iClient][iVictim] != 0.0)
			{
				g_flGhostHealStartTime[this.iClient][iVictim] = 0.0;
				g_iGhostHealStealCount[this.iClient][iVictim] = 0;
				g_iGhostHealGainCount[this.iClient][iVictim] = 0;
				Timer_EntityCleanup(null, g_iGhostParticleBeam[this.iClient][iVictim]);
			}
		}
		...
		if (this.flBuildingDrain > 0.0)
		{
			...
			while ((iBuilding = FindEntityByClassname(iBuilding, "obj_*")) > MaxClients)
			{
				...
				if (GetEntProp(iBuilding, Prop_Send, "m_iTeamNum") != iTeam)
				{
					...
					{
						bLinked = true;
						if (g_flGhostHealStartTime[this.iClient][iBuilding] == 0.0)
						{
							g_flGhostHealStartTime[this.iClient][iBuilding] = GetGameTime();
							...
							float vecTargetAngles[3];
							GetClientAbsAngles(this.iClient, vecTargetAngles);
							g_iGhostParticleBeam[this.iClient][iBuilding] = TF2_SpawnParticle(sParticle[iTeam], vecTargetOrigin, vecTargetAngles, true, iBuilding, EntRefToEntIndex(g_iGhostParticleCentre[this.iClient]));
						}
							
						float flTimeGap = GetGameTime() - g_flGhostHealStartTime[this.iClient][iBuilding];
							
						int iExpectedSteal = RoundToCeil(flTimeGap * flHealSteal * this.flBuildingDrain);
						if (iExpectedSteal > g_iGhostHealStealCount[this.iClient][iBuilding])
						{
							float flDamage = (float(iExpectedSteal - g_iGhostHealStealCount[this.iClient][iBuilding]));
							SDKHooks_TakeDamage(iBuilding, this.iClient, this.iClient, flDamage);
							g_iGhostHealStealCount[this.iClient][iBuilding] = iExpectedSteal;
						}
					}
						
					if (!bLinked && g_flGhostHealStartTime[this.iClient][iBuilding] != 0.0)
					{
						g_flGhostHealStartTime[this.iClient][iBuilding] = 0.0;
						g_iGhostHealStealCount[this.iClient][iBuilding] = 0;
						Timer_EntityCleanup(null, g_iGhostParticleBeam[this.iClient][iBuilding]);
					}
				}
			}
		}
		...
		if (g_flGhostLastSpookTime[this.iClient] < GetGameTime() - 1.5)
		{
			g_flGhostLastSpookTime[this.iClient] = GetGameTime();
	...
	{
		g_hGhostTimer[this.iClient] = null;
		SetEntProp(this.iClient, Prop_Data, "m_takedamage", DAMAGE_YES);
		
		Timer_EntityCleanup(null, g_iGhostParticleCentre[this.iClient]);
		for (int iEntity = 1; iEntity < 2048; iEntity++)
		{	
			if (g_flGhostHealStartTime[this.iClient][iEntity] > 0.0)
			{
				if (iEntity <= MaxClients)
					g_iGhostHealGainCount[this.iClient][iEntity] = 0;
				
				g_iGhostHealStealCount[this.iClient][iEntity] = 0;
				g_flGhostHealStartTime[this.iClient][iEntity] = 0.0;
				Timer_EntityCleanup(null, g_iGhostParticleBeam[this.iClient][iEntity]);
			}
		}
		...
		int iWeapon = GetEntPropEnt(this.iClient, Prop_Send, "m_hActiveWeapon");
	...
	public void OnButton(int &buttons)
	{
		...
		if (g_hGhostTimer[this.iClient] && buttons & IN_ATTACK)
			buttons &= ~IN_ATTACK;
	}
	...
	public void OnPlayerKilled(Event event)
	{
		...
		if (g_hGhostTimer[this.iClient] && iInflictor == this.iClient)
		{
